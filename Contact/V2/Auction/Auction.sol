...
contract Auction is Ownable {
    ...
    constructor () public {
          /* dayTime = 86400; */
          dayTime = 86400;
          inviteContract = Invite(0xceC072CE04b42b5019BC1E8A7091F90b87C7C14E);
          macTokenContract = ERC20(0x2700BC595607F154163471Ac2bD46eA8987d4cf0);
          magicNftContract = ERC721(0x6cd4d49EAc12020c0125Ce541cB068fC244bc3c1);
          platinumNftContract = ERC721(0xb772A41f8a27577D40924afa7D3cB3eEE9cbcE53);
          officialAddress = address(0x1f5a554fACfc45ab0089fe9eD38C46604E3C2A14);
          platinumShareAddress = address(0xA413c5be69cA0CbFa608a4DBe58B5e118BD15471);
    ...
    function exitAuctionByOfficial(uint256 _orderIndex) public returns (bool) {
          // Data validation
          ...
          if(order.status==1){
              ...
              macTokenContract.safeTransferFrom(address(this), order.account, profitAmount);// Transfer mac to online Address
              macTokenContract.safeTransferFrom(address(this), officialAddress, payFeeAmount.mul(40).div(100));// Transfer mac to official Address 40%
              macTokenContract.safeTransferFrom(address(this), platinumShareAddress, payFeeAmount.mul(60).div(100));// Transfer mac to platinumShare Address 60%

              if(order.nftType==1){
                  magicNftContract.sunshineTransferFrom(address(this),order.lastUser,order.tokenId);// Transfer nft1 to shop address
              }else{
                  platinumNftContract.sunshineTransferFrom(address(this),order.lastUser,order.tokenId);// Transfer nft2 to shop address
              }
          }else{
              if(order.nftType==1){
                  magicNftContract.sunshineTransferFrom(address(this),order.account,order.tokenId);// Transfer nft1 to me address
              }else{
                  platinumNftContract.sunshineTransferFrom(address(this),order.account,order.tokenId);// Transfer nft2 to me address
              }
          }

          auctionOrders[_orderIndex].status = 2;
          auctionOrders[_orderIndex].payFeeAmount = payFeeAmount;
          emit ExitAuction(order.account, _orderIndex, order.tokenId, order.lastAmount, payFeeAmount);// set log
    ...
    function exitAuction(uint256 _orderIndex) public returns (bool) {
          ...
          if(order.status==1){
              ...
              if(order.nftType==1){
                  magicNftContract.sunshineTransferFrom(address(this),address(msg.sender),order.tokenId);// Transfer nft1 to me address
              }else{
                  platinumNftContract.sunshineTransferFrom(address(this),address(msg.sender),order.tokenId);// Transfer nft2 to me address
              }
          }
